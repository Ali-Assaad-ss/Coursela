...
export default function VideoLesson({lesson}) {
  ...
  useEffect(()=>{if (lesson.content) setVUrl(`/api/file/lesson/${lesson.id}/file`)},[])
  const save=async () => {
  ...
  return (
    <>
      <h1 className="text-2xl font-bold m-5 mt-7 ml-10">Video Lesson</h1>
      <div className="flex flex-col p-16 gap-10 border rounded-xl mx-10">
        ...
        <div className="flex">
          ...
          <div className="border rounded-xl pb-5 w-[80%] ml-auto">
            <p className="border-4 rounded-t-xl text-center">Video</p>
            <Dropzone url={vUrl} setUrl={setVUrl} file={iFile} setFile={setIFile} type="video"/>
          </div>
        </div>
